...
const baseUrl = 'https://runes.lol';
const url = baseUrl + '/{gamemode}/platinum/plus/champion/{page}/{champion}/{role}';
const modeWin = 'win';
const modePick = 'pick';
const gamemodeRanked = 'ranked';
const gamemodeAram = 'aram';
...
function extractPage(html, pageName, pageUrl) {
	...
	$('.pure-u-8-24 > .runetitle').each(function () {
		keystones.push($(this).text());
	});
...
function _getPages(champion, callback) {
	...
	const pages = [];
	request(fillUrl(url, gamemodeRanked, modeWin, champion, ''), (error, response, html) => {
		if (!error && response.statusCode === 200) {
			...
			const $ = cheerio.load(html);
			const lanes = [];
			$('a.lanefilter').not('.active').each(function () {
				lanes.push($(this).attr('href'));
			});
			...
			for (let i = 0; i < pages.length; i++) {
				...
				request({url: pages[i].page, headers: {'X-PageName': pages[i].name}}, (error, response, html) => {
					if (!error && response.statusCode === 200) {
						const page = extractPage(html, response.request.headers['X-PageName'], response.request.uri.href);
						...
						if (count === 0) {
							const ordered = {};
							// We sort it back
							Object.keys(res.pages).sort().forEach(key => {
			...
			callback(res);
			throw new Error('failed to load main page for ' + champion);
		}
	});
}
...
const plugin = {
	...
	syncBookmark(bookmark, callback) {
		request(bookmark.src, (error, response, html) => {
			if (!error && response.statusCode === 200) {
				callback(extractPage(html, bookmark.pName, bookmark.src));
			} else {
				console.log(bookmark);
				throw new Error('unable to sync ' + bookmark.src);
			}
		});
	}
};
