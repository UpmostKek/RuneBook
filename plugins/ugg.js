...
async function getDataSource(champion) {
	try {
		...
		const lolVersion = lolVersions[0];
		const lolVersionUGG = getUGGFormattedLolVersion(lolVersion);
...
async function _getPages(champion, callback) {
	...
	try {
		...
		const totalGames = pages.reduce((total, current) => (total + current.games), 0);
